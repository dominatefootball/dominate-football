...
async function fetchTransfers(page = 1, blogsPerPage = 6) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/blogs?populate=*&filters[category][$eq]=transfers&pagination[start]=${start}&pagination[limit]=${blogsPerPage}&sort[0]=publishedDate:desc`,
    {
      headers: {
...
export default function TransfersPage() {
  ...
  useEffect(() => {
    async function loadBlogs() {
      try {
        const { blogs: blogData, totalBlogs } = await fetchTransfers(currentPage, blogsPerPage);
        setBlogs(blogData);
  ...
  return (
    <PageWrapper>
      <Header />
      <main className="p-10 min-h-screen">
        <div className="bg-white bg-opacity-90 shadow-lg rounded-lg p-6 max-w-7xl mx-auto overflow-visible">
          {/* Header with Title and Dropdown - responsive for longer titles */}
          <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-x-4 sm:space-y-0 mb-8">
            <h1 className="text-3xl font-bold text-gray-800 flex-shrink-0">Transfers</h1>

            {/* Club Filter Dropdown - stacked on mobile, inline on larger screens */}
            <select
              value={selectedClub}
              onChange={handleClubChange}
              className="px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors duration-200 cursor-pointer hover:border-red-400 text-sm min-w-40 w-full sm:w-auto"
            >
          ...
          <div className="block sm:grid sm:grid-cols-2 md:grid-cols-3 sm:gap-6 mb-8">
            {filteredBlogs.length === 0 ? (
              <div className="col-span-full text-center py-10 text-gray-500">
                No transfers found for {selectedClub}
              </div>
            ) : (
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center space-x-2 sm:space-x-4 mt-8">
              ...
              {/* Page Info */}
              <div className="bg-gray-100 px-3 py-1 sm:px-6 sm:py-2 rounded-lg shadow-sm text-xs sm:text-base">
